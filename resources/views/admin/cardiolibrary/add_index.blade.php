                ...
                <div class="card-body">
                    <form action="{{route('admin.cardio_library.store')}}" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="" class="form-control" placeholder="Enter Title" required>
                        </div>
                        ...
                        <div class="form-group">
                            <button type="button" id="importExercises" class="btn btn-primary ">Import</button>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <div>
                        <input type="radio" id="minutes" name="durationUnit" value="mins" checked>
                        <label for="minutes">Mins</label>
                        <input type="radio" id="seconds" name="durationUnit" value="secs">
    ...
    $(document).ready(function() {
        ...
        $('#importExercises').on('click', function() {
            ...
            exerciseTitles.forEach((exercise, index) => {
                exerciseListHtml += `
                    <li class="list-group-item d-flex justify-content-between bg-warning align-items-center mb-2" style="color:white;font-weight:bold;" data-id="${exercise.id}" data-unit="${exercise.unit}">
                        ${exercise.title}
                        <div>
                        <span class="badge badge-info">${exercise.duration} ${exercise.unit}</span>
        ...
        $('#saveDuration').on('click', function() {
            ...
            const unit = $('input[name="durationUnit"]:checked').val();
            ...
            $(`[data-id="${exerciseId}"] .badge`).text(`${duration} ${unit}`);
            $(`[data-id="${exerciseId}"]`).data('duration', duration);
            $(`[data-id="${exerciseId}"]`).data('unit', unit);
            
            // Clear the workout duration input field
            $('#workoutDuration').val("");
        ...
        $('form').on('submit', function(e) {
            ...
            $('#exerciseList li').each(function() {
                ...
                const unit = $(this).data('unit') || 'mins';
                exercises.push({id, duration, unit});
            });
