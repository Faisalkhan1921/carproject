...
Customer Routines
    <a href="{{route('admin.customer.routines.add')}}" class="btn btn-success btn-sm btn-sm"  id="btn-add"><i class="fa-solid fa-square-plus text-light"></i> Add New</a>
    ...
    <!-- <a href="#" class="btn btn-danger btn-sm" id="btn-bulk-delete"><i class="fa-solid fa-trash text-light"></i> Bulk Delete</a> -->
            ...
            <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h5 class="m-0 font-weight-bold text-primary">Customer Routines Records
                                        ...
                                        <tbody>
                                            ...
                                            @if(count($data) != 0)
                                                @foreach($data as $item)
                                                    @php 
                                                        $message = App\Models\Message::where('id', $item->user_id)->first();
                                                        if($message) {
                                                            $email = $message->email;
                                                            $fname = $message->fname; 
                                                            $lname = $message->lname; 
                                                        ...
                                                        $exercise_ids = App\Models\CustomerRoutineWorkouts::where('routine_id', $item->id)->pluck('exercise_id')->toArray();
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="{{ route('admin.customer.routines.view', $item->id) }}" class="btn btn-warning btn-sm">View</a>
                                                            <a href="{{ route('admin.customer.routines.edit', $item->id) }}" class="btn btn-primary btn-sm">Edit</a>
                                                            <a href="{{ route('admin.customer.routines.delete', $item->id) }}" onclick="return confirm('Are You Sure Want To Delete This?')" class="btn btn-danger btn-sm">Delete</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            @else 
...
function handleSubmit() {
    ...
    if (selectedAction === 'archive') {
        document.getElementById('archiveForm').action = "{{ route('admin.customer.routines.bulkdelete') }}";
