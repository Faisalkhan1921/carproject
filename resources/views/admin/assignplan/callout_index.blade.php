...
<div class="container mt-5">
    ...
    <ul class="nav nav-tabs" id="exerciseTabs" role="tablist">
        ...
        @if($warmup_emer->isEmpty())
            <a class="nav-link active" id="warmup-tab" data-toggle="tab" href="#warmup" role="tab" aria-controls="warmup" aria-selected="true">Warmup</a>
            @else 
            @php
                $warmup_counter = $warmup_emer->count();
            @endphp
            <a class="nav-link active" id="warmup-tab" data-toggle="tab" href="#warmup" role="tab" aria-controls="warmup" aria-selected="true">Warmup ({{ $warmup_counter }})</a>
            @endif
        </li>
        <li class="nav-item">
        @if($emer->isEmpty())
            <a class="nav-link" id="workout-tab" data-toggle="tab" href="#workout" role="tab" aria-controls="workout" aria-selected="false">Workout</a>
            @else 
    @php
        $counter = $emer->count();
    @endphp
    <a class="nav-link" id="workout-tab" data-toggle="tab" href="#workout" role="tab" aria-controls="workout" aria-selected="false">  Workout ({{ $counter }})</a>
    @endif
        </li>

        <li class="nav-item">
@if($cardio_emer->isEmpty() && $new_flag2->isEmpty())
            <a class="nav-link" id="cardio-tab" data-toggle="tab" href="#cardio" role="tab" aria-controls="cardio" aria-selected="false">Cardio</a>
            @else 
     @php
            $cardio_counter = $cardio_emer->count() + $new_flag2->count();

        @endphp   
        <a class="nav-link" id="cardio-tab" data-toggle="tab" href="#cardio" role="tab" aria-controls="cardio" aria-selected="false">Cardio ({{ $cardio_counter }})</a>
        @endif
        </li>
        <li class="nav-item">
        @if($cooldown_emer->isEmpty() && $new_flag1->isEmpty())
        <a class="nav-link" id="cooldown-tab" data-toggle="tab" href="#cooldown" role="tab" aria-controls="cooldown" aria-selected="false">Cooldown</a>
        @else 
                                    @php
                                        $cooldown_counter = $cooldown_emer->count() + $new_flag1->count();

                                    @endphp
            <a class="nav-link" id="cooldown-tab" data-toggle="tab" href="#cooldown" role="tab" aria-controls="cooldown" aria-selected="false">Cooldown ({{ $cooldown_counter }})</a>
            @endif
        </li>
    </ul>
...
@else
    ...
    @foreach($new_flag2 as $new_cardio)
        @php
            ...
            $exer_naming = $exer_name->title;
            $cardio_naming = App\Models\CardioModel::where('id', $new_cardio->cardio_id)->value('title');


            $routine_name = App\Models\CardioModel::where('id', $new_cardio->cardio_id)->first();
$cardio_rutine_naming1 = $routine_name->title;

// dd($cardio_rutine_naming1);
...
// echo "Success Progress: " . round($success_progress, 2) . "%";
        ...
        @if($currentcardioName2 !== $cardio_rutine_naming1)
            @if($currentcardioName2 !== null)
            ...
            <div class="col-md-12 mt-3">
                <div class="card border-0 shadow-sm rounded-lg">
                    <div class="card-header bg-danger text-white d-flex justify-content-between align-items-center">
                        <h6 class="mb-0"><i class="fas fa-dumbbell"></i>{{$cardio_rutine_naming1}} - {{$cardio_rutine_naming1}}</h6>
                        <span class="badge bg-light text-dark">Progress {{round($success_progress, 2) }} %</span>
                    </div>
                    <div class="card-body bg-light">
                        @php $currentcardioName2 = $cardio_rutine_naming1; @endphp
        @endif
        ...
        <div class="" >
            ...
            <a href="{{ route('admin.assignplan.view_cardioroutine_c', ['user_id' => $user , 'routine_id' => $new_cardio->cardio_id , 'increment' =>$increment, 'exercise' => $new_cardio->exercise_id,'row_id' => $new_cardio->id,'cardio_day' => $new_cardio->cardio_day]) }}" class="btn btn-danger btn-sm ">
                <i class="fas fa-info-circle"></i> More Info
            </a>
        </div>
        <hr>
...
@endif
    ...
    @foreach($cardio_emer as $emers5)
        @php
            ...
            $exer_naming = $exer_name->title;
            $cardio_naming = App\Models\CardioModel::where('id', $emers5->cardio_id)->value('title');
        @endphp

        @if($currentCardioName1 !== $cardio_naming)
            @if($currentCardioName1 !== null)
            </div> <!-- Close the last card body -->
    </div> <!-- Close the last card -->
    </div> <!-- Close the last card -->
            @endif
            ...
            <div class="col-md-12 mt-3">
                <div class="card border-0 shadow-sm rounded-lg">
                    <div class="card-header bg-danger text-white d-flex justify-content-between align-items-center">
                        <h6 class="mb-0"><i class="fas fa-dumbbell"></i> {{$cardio_naming}}</h6>
                        <span class="badge bg-light text-dark">Skipped</span>

                    </div>
                    <div class="card-body bg-light">
                        @php $currentCardioName1 = $cardio_naming; @endphp
        @endif
        ...
        <div class="mb-2">
            ...
            <a href="{{ route('admin.assignplan.view_cardioroutine_skip', ['user_id' => $user , 'routine_id' => $emers5->cardio_id , 'increment' =>$increment, 'exercise' => $emers5->exercise_id,'row_id' => $emers5->id]) }}" class="btn btn-danger btn-sm ">
                <i class="fas fa-info-circle"></i> More Info
            </a>
            <!-- <hr> -->
        </div>
...
@endif
        ...
        <!-- ///////////////////////////////////////////// start of cooldown //////////////////////////////////////////////////// -->
        <div class="tab-pane fade" id="cooldown" role="tabpanel" aria-labelledby="cooldown-tab">
            <h3>Cooldown Issues</h3>
            @if($cooldown_emer->isEmpty())
    <!-- Display message or empty state -->
@else
    @php
        $currentcooldownName1 = null;
    @endphp
    ...
    @foreach($cooldown_emer as $emers5)
        ...
        @if($currentcooldownName1 !== $cooldown_naming)
            @if($currentcooldownName1 !== null)
            ...
            <div class="col-md-12 mt-3">
                <div class="card border-0 shadow-sm rounded-lg">
                    ...
                    <div class="card-body bg-light">
                        @php $currentcooldownName1 = $cooldown_naming; @endphp
        @endif
