...
<hr>
    ...
    @endif
        ...
        <div class="col-md-12 mt-3">
            <div class="card border-0 shadow-sm rounded-lg">
                <div class="card-header bg-danger text-white d-flex justify-content-between align-items-center">
                <h6 class="mb-0"><i class="fas fa-heartbeat"></i>  Cardio </h6>
                <span class="badge bg-light text-dark">Callout</span>
    ...
    @else
                            @foreach($   as $emers3)
                                @php 
                                $cardio_name = App\Models\CardioWorkout::where('cardio_id',$emers3->cardio_id)->first();
                                $exer_name = App\Models\Exercise::where('id',$emers3->exercise_id)->first();
                               // dd($emers3->cardio_id);
                                    $exer_naming = $exer_name->title;
                                    $cardio_naming = App\Models\CardioModel::where('id', $emers3->cardio_id)->value('title');
                                @endphp
                            
                                <div class="col-md-12 mt-3">
    <div class="card border-0 shadow-sm rounded-lg">
        <div class="card-header bg-danger text-white d-flex justify-content-between align-items-center">
            <h6 class="mb-0"><i class="fas fa-heartbeat"></i> {{$cardio_naming}} - {{$exer_naming}}</h6>
            <a href="{{ route('admin.assignplan.view_cardioroutine', ['user_id' => $user , 'routine_id' => $emers3->cardio_id , 'increment' =>$increment]) }}" class="btn btn-danger btn-sm ">
                <i class="fas fa-info-circle"></i> More Info
            </a>
            
        </div>
        <div class="card-body bg-light">
            <p class="mb-2"><strong>Emergency Stop:</strong> {{$emers3->emergency_stop}}</p>
