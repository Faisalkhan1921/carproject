...
<div class="container-fluid">
        ...
        <div class="col-md-12  m-auto">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h5 class="m-0 font-weight-bold text-primary">Edit User Routine Library
                        <a href="{{ route('admin.module.library.routines.index') }}" class="btn btn-warning btn-sm float-right">Return to List</a>
                    </h5>
                </div>
                <div class="card-body">
                    <form id="routineForm" action="{{ route('admin.module.library.routines.update1', $data->id) }}" method="POST">
                        @csrf
    ...
    <div class="workout-item" data-id="{{ $workout->id }}">
        ...
        <p style="margin: none; padding:none;display:inline-block;"> {{ $workout->title }} - {{ $workout->exercise->title }}</p>
        <input type="hidden" name="workouts[]" value="{{ $workout->title }}-{{ $workout->exercise_id }}">
            <a href="#" class="edit-workout" data-id="{{ $workout->id }}" data-title="{{ $workout->title }}" data-exercise="{{ $workout->exercise_id }}"><i class="fas fa-edit"></i></a>
    ...
    @endforeach
                        ...
                        <input type="submit" value="Update User Routine Library" class="form-control btn btn-primary">
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('admin.module.library.routines.update',$data2->id)}}" id="editWorkoutForm" method="post">
                    @csrf
                    <input type="hidden" id="editWorkoutId">
                    <div class="form-group">
                        <label for="edit_routine_workout_title">Workout Title</label>
                        <input type="text" name="routine_workout_title" id="edit_routine_workout_title" class="form-control" placeholder="Workout Title">
                    </div>
                    <div class="form-group">
                    ...
                    <!-- <button type="button" class="btn btn-primary float-right" id="updateWorkoutBtn">Update Workout</button> -->
                     <input type="submit" value="Update Workout" class="btn btn-primary float-right">
